...
void prettyprint_expression(expression_t* expression, long tab_count) {
	...
	if (expression->type == et_integer_literal) {
		...
		std::cout << tabs << T << "parameters: {" << std::endl;
		for (int i = 0; i < expression->function_call.parameters.size(); i++) {
			prettyprint_expression(expression->function_call.parameters[i], tab_count + 2);
			if (i != expression->function_call.parameters.size() - 1) {
				std::cout << ",";
			}
			std::cout << std::endl;
		}
